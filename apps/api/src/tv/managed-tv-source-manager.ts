...
export interface ManagedTvSourceUpdate {
  enabled?: boolean;
  priority?: number;
}
...
export async function importManagedTvPlaylist(input: ManagedTvImportInput): Promise<ManagedTvImportResult> {
  ...
  if (!sourceKey) throw invalid('sourceKey must contain ASCII letters or numbers');

  const bytes = Buffer.byteLength(input.playlistText, 'utf8');
  if (bytes === 0) throw invalid('Playlist is empty');
  if (bytes > MAX_PLAYLIST_BYTES) throw invalid('Playlist exceeds 20 MB import limit');

  const parsed = parseM3u(input.playlistText);
  if (parsed.entries.length === 0) throw invalid('Playlist contains no playable HTTP/HTTPS stream entries');
  ...
  return db.transaction(async (tx) => {
    ...
    const dynamicSeeds = dedupeSeeds(
      imported.map((item) => item.seed).filter((seed) => !INITIAL_BY_KEY.has(seed.canonicalKey)),
    );
    if (dynamicSeeds.length > 0) await seedCatalog(tx, dynamicSeeds);

    const groups = groupImportedChannels(imported);
    const canonicalIds = await loadCanonicalIds(tx, groups.map((item) => item.seed.canonicalKey));
    const repository = new TvCatalogRepository(tx);

    // Re-importing one source atomically replaces only that source provider's bindings.
    // Other provider sources remain intact and continue to serve as fallbacks.
    await tx.query(`
      delete from control.playback_bindings
      where provider_id = $1 and binding_type = 'IPTV_STREAM'
    `, [providerId]);

    const tvgCounts = countMeaningfulTvgIds(parsed.entries);
    let boundChannelCount = 0;

    for (const group of groups) {
      const entityId = canonicalIds.get(group.seed.canonicalKey);
      if (!entityId || group.entries.length === 0) continue;

      const representative = preferredEntry(group.entries);
      await upsertProviderRef(tx, providerId, entityId, sourceKey, representative, group.seed);
      await repository.replacePlaybackBindings(
        entityId,
        providerId,
        descriptorFromEntries(group.entries, providerId, sourceKey),
      );
      ...
      await updateChannelMetadata(tx, entityId, group.seed, representative);
      const epgId = uniqueEpgId(group.entries, tvgCounts);
      if (epgId) await upsertEpgMapping(tx, entityId, providerId, epgId);
      boundChannelCount += 1;
    }
    ...
    await upsertPlaylistControl(tx, {
      ...
      sourceKey,
      fingerprint,
      ...(parsed.epgUrls[0] ? { epgSourceRef: parsed.epgUrls[0] } : {}),
      publish,
...
export async function updateManagedTvSource(sourceKeyInput: string, update: ManagedTvSourceUpdate) {
  const sourceKey = normalizeSourceKey(sourceKeyInput);
  if (!sourceKey) throw invalid('Invalid sourceKey');
  if (update.enabled === undefined && update.priority === undefined) {
    throw invalid('At least one of enabled or priority is required');
  }

  const providerCode = providerCodeForSource(sourceKey);
  const db = getDatabase();
  const provider = (await db.query<IdRow>(`
    select id from control.providers where code = $1 limit 1
  `, [providerCode]))[0];
  if (!provider) {
    throw new ProviderError({
      providerId: 'admin-tv-source',
      code: 'NOT_FOUND',
      message: `TV source ${sourceKey} was not found`,
      retryable: false,
      statusCode: 404,
    });
  }
  ...
  if (update.priority !== undefined) {
    const priority = clampInt(update.priority, 100, 1, 100000);
    await db.query(`update control.providers set priority = $2, updated_at = now() where id = $1`, [provider.id, priority]);
  }

  if (update.enabled !== undefined) {
    await db.query(`update control.providers set enabled = $2, updated_at = now() where id = $1`, [provider.id, update.enabled]);
    await db.query(`update control.iptv_playlists set enabled = $2, updated_at = now() where provider_id = $1`, [provider.id, update.enabled]);
    await db.query(`
      update control.playback_bindings
      set enabled = $2, updated_at = now()
      where provider_id = $1 and binding_type = 'IPTV_STREAM'
    `, [provider.id, update.enabled]);
  }
  ...
  return {
    ...
    ...(update.priority !== undefined ? { priority: clampInt(update.priority, 100, 1, 100000) } : {}),
  };
}

export async function setManagedTvSourceEnabled(sourceKey: string, enabled: boolean): Promise<void> {
  await updateManagedTvSource(sourceKey, { enabled });
}
...
async function updateChannelMetadata(db: SqlExecutor, entityId: string, seed: InitialTvChannelSeed, entry: M3uEntry): Promise<void> {
  ...
  `, [
    entityId,
    entry.logoUrl ?? seed.logoUrl ?? null,
    entry.isHd || seed.isHd,
    JSON.stringify({ latestTvgId: entry.externalId, sourceGroup: entry.group ?? seed.sourceGroup ?? null }),
  ]);
}

async function upsertEpgMapping(db: SqlExecutor, entityId: string, providerId: string, epgId: string): Promise<void> {
  await db.query(`
...
async function upsertPlaylistControl(db: SqlExecutor, options: {
  ...
  if (existing) {
    await db.query(`
      ...
      set name = $3,
          source_type = 'M3U_FILE',
          source_ref = $4,
          epg_source_ref = $5,
          enabled = $6,
          refresh_interval_minutes = $7,
          last_sync_at = now(),
          next_sync_at = now() + ($7::int * interval '1 minute'),
          updated_at = now()
    ...
    `, [
      ...
      options.providerId,
      options.sourceKey,
      `admin-upload:${options.fingerprint}`,
      options.epgSourceRef ?? null,
      options.publish,
      options.refreshIntervalMinutes,
    ]);
    return;
  }
  ...
  await db.query(`
    insert into control.iptv_playlists (
      ...
      $1, $2, 'M3U_FILE', $3, $4, $5, $6,
      now(), now() + ($6::int * interval '1 minute')
    )
  `, [
    options.providerId,
    options.sourceKey,
...
async function storeEpgSources(db: SqlExecutor, providerId: string, sourceKey: string, urls: readonly string[]): Promise<void> {
  await db.query(`
    ...
    ) values ($1, 'TV', 'EPG_SOURCES', 1, $2::jsonb, true)
    on conflict (provider_id, domain, operation, mapping_version) do update set
      mapping_json = excluded.mapping_json,
      enabled = true,
      updated_at = now()
  `, [providerId, JSON.stringify({ sourceKey, urls })]);
}
...
function groupImportedChannels(items: readonly ImportChannel[]): ImportChannelGroup[] {
  const groups = new Map<string, ImportChannelGroup>();
  for (const item of items) {
    const current = groups.get(item.seed.canonicalKey);
    if (current) current.entries.push(item.entry);
    else groups.set(item.seed.canonicalKey, { seed: item.seed, entries: [item.entry] });
  }
  return [...groups.values()];
}

function descriptorFromEntries(entries: readonly M3uEntry[], providerId: string, sourceKey: string): PlaybackDescriptor {
  const candidates = entries.map((entry): PlaybackCandidate => ({
...
function preferredEntry(entries: readonly M3uEntry[]): M3uEntry {
  return [...entries].sort((left, right) => {
    const leftScore = Number(Boolean(left.logoUrl)) + Number(Boolean(meaningfulTvgId(left.externalId))) + Number(left.isHd);
    const rightScore = Number(Boolean(right.logoUrl)) + Number(Boolean(meaningfulTvgId(right.externalId))) + Number(right.isHd);
    return rightScore - leftScore;
  })[0]!;
}

function countMeaningfulTvgIds(entries: readonly M3uEntry[]): Map<string, number> {
...
function uniqueEpgId(entries: readonly M3uEntry[], counts: ReadonlyMap<string, number>): string | undefined {
  for (const entry of entries) {
    const id = meaningfulTvgId(entry.externalId);
    if (id && counts.get(id.toLowerCase()) === 1) return id;
  }
  return undefined;
}

export function providerCodeForSource(sourceKey: string): string {
  const normalized = normalizeSourceKey(sourceKey).toUpperCase().replace(/[^A-Z0-9]+/g, '_').replace(/^_+|_+$/g, '');
  const prefix = normalized.slice(0, 40) || 'SOURCE';
  return `IPTV_${prefix}_${stableHash(sourceKey).toUpperCase()}`;
}
